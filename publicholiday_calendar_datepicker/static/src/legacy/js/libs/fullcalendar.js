odoo.define('publicholiday_calendar_datepicker.fullcalendar', function (require) {
    'use strict';

    var fullcalendar = require('/web/static/src/js/libs/fullcalendar.js');
            ...
            _setCursorOnEventDates() {
                const request = async () => {
                    ...
                    for (const event of Object.values(this.events)) {
                        const response = await fetch('/publicholiday_calendar_datepicker/is_holiday?event_id=' + event.id)
    ...
    fullcalendar.createYearCalendarView = createYearCalendarView;
});
